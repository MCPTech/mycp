...
//>>>>>>> origin/development
	...
	@RemoteMethod
	public List<VolumeInfoP> findAll4List4Infra(Infra infra) {
		...
		try {
			if (Commons.EDITION_ENABLED == Commons.SERVICE_PROVIDER_EDITION_ENABLED  || 
				...
				if (Commons.getCurrentUser().getRole().getName().equals(Commons.ROLE.ROLE_SUPERADMIN + "")) {
					return  VolumeInfoP.findVolumeInfoPsByInfra(infra).getResultList();
					...
					return VolumeInfoP.findVolumeInfoPsBy(infra, Company.findCompany(Commons.getCurrentSession()
