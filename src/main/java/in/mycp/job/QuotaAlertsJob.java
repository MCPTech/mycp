...
public class QuotaAlertsJob implements EventListener {
	private static final Logger logger = Logger.getLogger(QuotaAlertsJob.class.getName());
		...
		try {
			...
			for (Company company : lstCompany) {
				...
				try {
					...
					try {
						List<User> managers = User.findManagersByCompany(company).getResultList();
						for (Iterator iterator = managers.iterator(); iterator.hasNext();) {

							try {
								User user2 = (User) iterator.next();
								accountLogService.saveLogAndSendMail(ex.getMessage(), "Quota Regular Check Job", Commons.task_status.FAIL.ordinal(), user2.getEmail());
							} catch (Exception e) {
