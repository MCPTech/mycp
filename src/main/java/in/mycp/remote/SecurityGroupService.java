		...
		try {
			...
			if (instance.getId() != null && instance.getId() > 0) {
				...
				if (true == assetTypeSecurityGroup.getWorkflowEnabled()) {
					...
					accountLogService.saveLog("Security Group with ID "+instance.getId()+" requested, workflow started, pending approval.", Commons.task_name.SECURITYGROUP.name(), 
					...
					accountLogService.saveLog("Security Group with ID "+instance.getId()+" requested, workflow approved automatically.", Commons.task_name.SECURITYGROUP.name(), 
							Commons.task_status.SUCCESS.ordinal(),currentUser.getEmail());
